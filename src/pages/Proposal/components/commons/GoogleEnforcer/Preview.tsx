...
function PreviewOptionUploaded(props: Props) {
  const { setIsModalPreview, previewData, handleRemoveOptions, content } = props;
      ...
      <table className="w-full">
        <thead>
          <tr className="border-solid border-b border-[#F6F6F6]">
            ...
            <td className="pl-4 text-start pb-[14px]">Email address</td>
          </tr>
        </thead>
        <tbody>
          {previewData.map((item: any, index: number) => (
            ...
            <tr key={index} className="border-b h-[55px] border-solid border-b-[#F6F6F6]">
              {content && <td className="pl-4 text-[#252422] font-medium text-base">{item[0]}</td>}
              <td className="pl-4 text-start text-[#252422] font-medium text-base">
                {content ? item[1] : item}
              </td>
