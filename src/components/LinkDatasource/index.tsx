...
function LinkDatasource(props: Props) {
  ...
  function getPreviewData(
    ...
    const data = rows.map((row: any) => [row[column]]).filter((row) => row[0] !== undefined);
  ...
  const handleUploadFile = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        setWorkbook(wb);
        setPreviewData(
          getPreviewData(wb, sheetNameDropdown[0].label, columnsOption, isIgnoreHeader),
        );
      };
  ...
  const handleOnChangeSheet = (sheetSelected: any) => {
    if (sheetSelected && sheetSelected.label !== activeSheet.label) {
      ...
      setPreviewData(getPreviewData(workbook, sheetSelected?.label, columnsOption, isIgnoreHeader));
    }
  };
  ...
  const handleChangeOptionColumn = (value: string) => {
    ...
    if (activeSheet && workbook) {
      setPreviewData(
        getPreviewData(workbook, activeSheet.label, value.toUpperCase(), isIgnoreHeader),
      );
    }
  };
  ...
  const handleOnchangeToggle = () => {
    ...
    if (activeSheet && workbook) {
      setPreviewData(getPreviewData(workbook, activeSheet.label, columnsOption, !isIgnoreHeader));
    }
  };
      ...
      {isModalPreview ? (
        ...
        <>
          <div className="flex justify-between items-center mb-[34px]">
            <div className="text-[#252422] text-2xl font-semibold ">{L('linkDatasource')}</div>
            <div
          ...
          {fileName && (
            <>
              ...
              {/*    <input */}
              {/*      type="text" */}
              {/*      value={columnsEmail?.trim().toUpperCase()} */}
              {/*      onChange={(e: any) => setColumnsEmail(e.target.value)} */}
